...
- **Queued follow-up**: Each task may have one queued message in SQLite (`task_message_queue`). The client hydrates it after reload, while the server dispatches it through the normal message route when the active run or compaction settles—even with no browser tab open. An accepted send atomically consumes the matching queue row; failed startup restores it without overwriting a replacement. Editing, steering, and removal also compare queue IDs so stale tabs cannot erase newer work.
