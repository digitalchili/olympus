...
import { trackTaskRun } from '../task-run-lifecycle.js';
...
import { captureCodingBaseline, readCodingEvidence, verifyCodingRun } from '../coding-verification.js';
...
codingVerificationRouter.post('/:id/verification', async (_req, res) => {
  ...
  if (['streaming','compacting'].includes(getRunStatus(task.id)?.status ?? '')) return res.status(409).json({ error: 'Wait for the active run to settle.' });
  ...
  try {
    await captureCodingBaseline(task, run.runId);
    if (['streaming','compacting'].includes(getRunStatus(task.id)?.status ?? '')) return res.status(409).json({ error: 'A new run started; verification was not started.' });
