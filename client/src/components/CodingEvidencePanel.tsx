    ...
    {visibleEvidence && evidence && <div className="flex items-start gap-3">
      ...
      {!running && <ul className="mt-2 space-y-2">{evidence.checks.map((check, i) => {
        ...
        return <li key={`${i}:${evidence.updatedAt}`}>
          <details open={failed || undefined}><summary className="cursor-pointer break-all font-medium">{failed ? 'Failed' : 'Passed'}: {check.command.join(' ')} · {(check.durationMs / 1000).toFixed(1)}s</summary>
            {failed && output.length > 6000 && <p className="mt-1 text-zinc-500">End of command output:</p>}
            <pre ref={failed ? showOutputEnd : undefined} className="mt-1 max-h-32 overflow-auto whitespace-pre-wrap break-all">{failed ? output.slice(-6000) : output}</pre>
            {failed && output.length > 6000 && <details className="mt-2"><summary className="cursor-pointer underline">Full command output</summary><pre className="mt-1 max-h-32 overflow-auto whitespace-pre-wrap break-all">{output}</pre></details>}
          </details>
        </li>;
      })}</ul>}
