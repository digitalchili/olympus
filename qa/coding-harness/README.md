...
POST `/qa/checks-slow`, click **Run checks**, then **Stop checks**. Verify the panel reports the user stop, the task stays in progress, and another check can run. The conflict URL uses a real disposable upstream and Project branch. Send a message and verify that the inline error names `source.txt`, explains recovery, and preserves the unsent draft; repeated sends must not start an agent or change editor ownership.
